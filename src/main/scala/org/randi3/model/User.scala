...
case class User private(id: Int, version: Int, username: String, password: String, email: String, firstName: String, lastName: String, phoneNumber: String, site: TrialSite, rights: Set[TrialRight], administrator: Boolean, canCreateTrial: Boolean, locale: Locale, private val dummy: Any) extends Entity {
...
object User {
  ...
  * */
  def apply(id: Int = Int.MinValue, version: Int = 0, username: String, password: String, email: String, firstName: String, lastName: String, phoneNumber: String, site: TrialSite, rights: Set[TrialRight], administrator: Boolean = false, canCreateTrial: Boolean = false, locale: Locale = Locale.ENGLISH): ValidationNEL[String, User] = {
    checkAll(
      checkID(id),
      ...
      case Right(_) => Success(new User(id, version, username, hashPassword(username, password), email, firstName, lastName, phoneNumber, site, rights, administrator, canCreateTrial, locale, null))
    }
  }

  private def validUser = new User(Int.MinValue, 0, "validName", "validPassword", "valid@mail.de", "validFirst", "validLastName", "123456", TrialSite(Int.MinValue, 0, "validName", "validCountry", "validStreet", "validPostCode", "validCity", "validPassord").toOption.get, Set(), false, false, Locale.ENGLISH, null)

...
trait UtilityMailComponent extends Utility {
  ...
  class UtilityMail {
    ...
    def getRandomizedMailContent(trial: Trial, treatmentArm: TreatmentArm, trialSubject: TrialSubject): String = {
      ...
      val st = new ST(getMail("mail.randomized", securityUtility.currentUser.get), '$', '$')
    ...
    *
    */
    private def getMail(mailType: String, user: User): String = {
      ...
      st.add("username", user.firstName + " " + user.lastName)
